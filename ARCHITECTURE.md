...
what breaks if you change it. It is the page to read before a pull request, and the page to read when
a vLLM upgrade turns something red.
...
**The weight-loading trap, and it is the sharpest edge in the repo.** `stacked_params_mapping` is
...
number — the retry baseline decodes the same way) or greedily (`greedy`, which isolates the
...
| change how an operator steers | `processors/logit_repair.py` | it is in the hot path, and it changes what published numbers mean — open an issue first |
