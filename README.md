...
`examples/showcase_three_regimes.py` takes problems the specialist fails on every sampled rollout
and classifies what (if anything) rescues each, alongside the junction-feature profile. **The output
below is real** (regenerated by the script on `Qwen/Qwen3-0.6B` + `Qwen/Qwen3-0.6B-Base`):
