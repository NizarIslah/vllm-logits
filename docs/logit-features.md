...
| `logit_var` | `Var_{y∼p_S^T}[ z_S(y) ]` over the specialist's top-`K` (probability-weighted variance of the logits) | **Logit dispersion / temperature sensitivity.** The Fisher information of the temperature submodel ([ref](https://arxiv.org/abs/2602.14862)). High ⇒ a peaked, confident distribution that responds strongly to a temperature change. |
